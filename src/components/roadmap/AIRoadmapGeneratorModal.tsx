...
import { generateAndChatRoadmap } from '../../services/ai';
import type { ChatGeneratedMilestone } from '../../services/ai';
import { Bot, User, Send, CheckCircle, Loader2, Minimize2, Sparkles, Map } from 'lucide-react';
import type { Milestone } from '../../types/roadmap';

interface AIRoadmapGeneratorModalProps {
  isOpen: boolean;
  onClose: () => void;
  targetRole: string;
  currentMilestones: Milestone[];
  onConfirm: (milestones: ChatGeneratedMilestone[], replaceFuture: boolean) => void;
}
...
export const AIRoadmapGeneratorModal: React.FC<AIRoadmapGeneratorModalProps> = ({
  isOpen,
  onClose,
  targetRole,
  currentMilestones,
  ...
  const [proposedMilestones, setProposedMilestones] = useState<ChatGeneratedMilestone[]>([]);
  ...
  const handleSendInit = async () => {
    setLoading(true);
    try {
      const response = await generateAndChatRoadmap(targetRole, currentMilestones.map(m => m.title), '', []);
      if (response) {
        setMessages([
          { id: `ai-${Date.now()}`, sender: 'ai', text: response.chat_response }
        ]);
        setProposedMilestones(response.proposed_milestones);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const response = await generateAndChatRoadmap(targetRole, currentMilestones.map(m => m.title), userText, chatHistory);
      if (response) {
        setMessages(prev => [
          ...prev, 
          { id: `ai-${Date.now()}`, sender: 'ai', text: response.chat_response }
              ...
              {proposedMilestones.map((ms, idx) => (
                <div key={idx} style={{ border: '1px solid var(--border-color)', borderRadius: '12px', padding: '16px', background: '#f8fafc' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                    <div style={{ width: '40px', height: '40px', borderRadius: '8px', background: '#e0e7ff', color: '#4f46e5', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', fontWeight: 'bold' }}>
                      {idx + 1}
                    </div>
                    <div>
